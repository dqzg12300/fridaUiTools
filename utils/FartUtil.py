...
methodTable={}
TMP_DATA=""
def appentOutRes(data, isLine=True):
	global LOG_SIGNEL
	global TMP_DATA
...
def get_uleb128(content):
	...
	if len(content) < 5:
		for i in range(0, len(content)):
			tmp = int(content[i]) & 0x7f
			value = tmp << (i * 7) | value
			if (int(content[i]) & 0x80) != 0x80:
				break
	elif len(content) == 5:
		for i in range(0, 5):
			tmp = int(content[i]) & 0x7f
	...
	if i == 4 and (tmp & 0xf0) != 0:
		appentOutRes("parse a error uleb128 number")
		return -1
	return i + 1, value
...
def parse_FMT10X(buffer, dex_object, pc_point, offset):
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1])


def parse_FMT10T(buffer, dex_object, pc_point, offset):
	val, = struct.unpack_from("b", buffer, 1)
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "%04x" % (val + offset))


def parse_FMT11N(buffer, dex_object, pc_point, offset):
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % (int(buffer[1]) & 0xf),
			"%d" % ((int(buffer[1]) >> 4) & 0xf))


def parse_FMT11X(buffer, dex_object, pc_point, offset):
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % int(buffer[1]))


def parse_FMT12X(buffer, dex_object, pc_point, offset):
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % (int(buffer[1]) & 0x0f),
			"v%d" % ((int(buffer[1]) >> 4) & 0xf))


def parse_FMT20T(buffer, dex_object, pc_point, offset):
	v, = struct.unpack_from("h", buffer, 2)
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "%04x" % (v + offset))
...
def parse_FMT22B(buffer, dex_object, pc_point, offset):
	...
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % int(buffer[1]), "v%d" % bb, "%d" % cc)


def parse_FMT22C(buffer, dex_object, pc_point, offset):
	cccc, = struct.unpack_from("H", buffer, 2)
	if int(buffer[0]) == 0x20 or int(buffer[0]) == 0x23:
	...
	bb = int(buffer[1]) >> 4
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % (int(buffer[1]) & 0xf),
			"v%d" % ((int(buffer[1]) >> 4) & 0xf), "%s" % prefix)
...
def parse_FMT22T(buffer, dex_object, pc_point, offset):
	...
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % (int(buffer[1]) & 0xf),
			"v%d" % ((int(buffer[1]) >> 4) & 0xf), "%04x" % (cccc + offset))
...
def parse_FMT23X(buffer, dex_object, pc_point, offset):
	cc, bb, = struct.unpack_from("Bb", buffer, 2)
	return (
	dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % int(buffer[1]), "v%d" % bb, "v%d" % cc)


def parse_FMT30T(buffer, dex_object, pc_point, offset):
	aaaaaaaa, = struct.unpack_from("i", buffer, 2)
	return dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "+%x" % (aaaaaaaa + offset)


def parse_FMT31C(buffer, dex_object, pc_point, offset):
	bbbbbbbb, = struct.unpack_from("I", buffer, 2)
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % int(buffer[1]), "+%d" % bbbbbbbb)
...
def parse_FMT31T(buffer, dex_object, pc_point, offset):
	bbbbbbbb, = struct.unpack_from("i", buffer, 2)
	return (
	dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % int(buffer[1]), "string@%d" % bbbbbbbb)


def parse_FMT32X(buffer, dex_object, pc_point, offset):
	aaaa, bbbb, = struct.unpack_from("hh", buffer, 2)
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % aaaa, "v%d" % bbbb)
...
def parse_FMT35C(buffer, dex_object, pc_point, offset):
	...
	if A == 5:
		return (
		dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % C, "v%d" % D, "v%d" % E, "v%d" % F,
		"v%d" % G, "%s" % (prefix))
	elif A == 4:
		return (
		dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % C, "v%d" % D, "v%d" % E, "v%d" % F,
		...
		return (
		dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % C, "v%d" % D, "v%d" % E, "%s" % (prefix))
	elif A == 2:
	...
	return (
	dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % C, "v%d" % D, "v%d" % E, "v%d" % F, "v%d" % G,
	"%s" % (prefix))
...
def parse_FMT51L(buffer, dex_object, pc_point, offset):
	...
	return (dex_decode[int(buffer[0])][4], dex_decode[int(buffer[0])][1], "v%d" % int(buffer[1]), "%d" % bb)
...
def parse_instruction(buffer, offset, dex_object):
	...
	while start < n:
		if n == 1736:
			appentOutRes( "start = %d" % start)
		op = int(buffer[start])
		if op == 0:
			type = int(buffer[start + 1])
			if type == 1:
		...
		val = func_point[dex_decode[op][3]](buffer[start:], dex_object, offset + start, int(start / 2))
		str = ""
		m = 0
		...
		appentOutRes( "%08x: %-36s |%04x:" % (offset + start, str, int(start / 2)),False)
		m = 0
		for v in val[1:]:
			if m > 1:
				appentOutRes( ",",False)
			appentOutRes( v,False)
			m += 1
		appentOutRes( "")
		start += 2 * val[0]
...
def parseinsfile(insfilename):
	insfile=open(insfilename,"r",encoding="utf-8")
...
class method_code:
	...
	def printf(self,dex_object,prefix=""):
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"registers_size",self.registers_size,self.registers_size))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"insns_size",self.insns_size,self.insns_size))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"debug_info_off",self.debug_info_off,self.debug_info_off))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"ins_size",self.ins_size,self.ins_size))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"outs_size",self.outs_size,self.outs_size))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"tries_size",self.tries_size,self.tries_size))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"insns",self.insns,self.insns))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"tries",self.tries,self.tries))
		appentOutRes( "%s%-20s:%08x:%10d"%(prefix,"handlers",self.handlers,self.handlers))
...
class dex_encode_method:
	...
	def printf(self,dex_object):
		...
		appentOutRes( "%-20s%08x %s"%("m_method_idx_diff",self.m_method_idx_diff,name))
		flags = dex_object.get_access_flags(self.m_access_flags)
		appentOutRes( "%-20s%08x %s"%("access_flags",self.m_access_flags,flags))
		appentOutRes( "%-20s%08x %d"%("code_off",self.m_code_off,self.m_code_off))
...
class dex_class:
	...
	def printf(self,dex_object):
		#打印class_def相关的信息
		appentOutRes("%-20s:%08x:%10d  %s"%("thisClass",self.thisClass,self.thisClass,dex_object.gettypename(self.thisClass)))
		appentOutRes("%-20s:%08x:%10d  %s"%("superClass",self.superClass,self.superClass,dex_object.gettypename(self.superClass)))
		appentOutRes("%-20s:%08x:%10d"%("modifiers",self.modifiers,self.modifiers))
		appentOutRes("%-20s:%08x:%10d"%("offset",self.offset,self.offset))
		appentOutRes("%-20s:%08x:%10d"%("annotationsOff",self.annotationsOff,self.annotationsOff))
		appentOutRes("%-20s:%08x:%10d"%("numStaticFields",self.numStaticFields,self.numStaticFields))
		appentOutRes("%-20s:%08x:%10d"%("numInstanceFields",self.numInstanceFields,self.numInstanceFields))
		appentOutRes("%-20s:%08x:%10d"%("numDirectMethods",self.numDirectMethods,self.numDirectMethods))
		appentOutRes("%-20s:%08x:%10d"%("numVirtualMethods",self.numVirtualMethods,self.numVirtualMethods))
		appentOutRes("%-20s:%08x:%10d"%("classDataOff",self.classDataOff,self.classDataOff))
		appentOutRes("%-20s:%08x:%10d"%("interfacesOff",self.interfacesOff,self.interfacesOff))
		appentOutRes("%-20s:%08x:%10d"%("interfacesSize",self.interfacesSize,self.interfacesSize))
		...
		for n in range(0,self.interfacesSize):
			...
			appentOutRes("\t\t"+ dex_object.gettypename(typeid))

		appentOutRes( "%-20s:%08x:%10d"%("staticValuesOff",self.staticValuesOff,self.staticValuesOff))
		appentOutRes( "%-20s:%08x:%10d  %s"%("sourceFileIdx",self.sourceFileIdx,self.sourceFileIdx,dex_object.getstringbyid(self.sourceFileIdx)))
		#这里取出classData中的静态字段大小，成员字段大小，静态方法大小，成员方法大小
		offset = self.classDataOff
		n,tmp = get_uleb128(dex_object.m_content[offset:offset+5])
		offset += n
		n,tmp = get_uleb128(dex_object.m_content[offset:offset+5])
		...
		for i in range(0,self.numStaticFields):
			...
			offset += n
			field_idx+=field_idx_diff
			appentOutRes( dex_object.getfieldfullname(field_idx),False)
			n,modifiers = get_uleb128(dex_object.m_content[offset:offset+5])
			offset += n
			#如果静态字段有值
			if self.staticValuesOff:
		...
		for i in range(0,self.numInstanceFields):
			...
			offset += n
			field_idx+=field_idx_diff
			appentOutRes( dex_object.getfieldfullname(field_idx))
			n,modifiers = get_uleb128(dex_object.m_content[offset:offset+5])
			offset += n

		appentOutRes( "=========numDirectMethods[%d]=numVirtualMethods[%d]=numStaticMethods[0]========="%(self.numDirectMethods,self.numVirtualMethods))
		method_idx = 0
		#遍历实例函数
		for i in range(0,self.numDirectMethods):
			n,method_idx_diff = get_uleb128(dex_object.m_content[offset:offset+5])
			...
			if code_off != 0:
				...
				if method != None:
					...
					try:
						appentOutRes( "before repire method+++++++++++++++++++++++++++++++++++\n")
						#打印一下这个method_code
						method_code(dex_object, code_off).printf(dex_object, "\t\t")
					except Exception as e:
						appentOutRes( str(e))
					try:
						#获取.bin文件中的code_item的数据，用base64解码一下拿到二进制数据
						bytearray_str = base64.b64decode(method.insarray)
						appentOutRes( "after repire method++++++++++++++++++++++++++++++++++++\n")
						#用上面的二进制数据生成code_item。并打印
		...
		if self.annotationsOff != 0:
			...
			if self.class_annotations_off == 0:
				return
			appentOutRes( "self.class_annotations_off = %x"%self.class_annotations_off)
	...
	def repiredex(self, dex_object):
		...
		appentOutRes( "=========numDirectMethods[%d]=numVirtualMethods[%d]=numStaticMethods[0]=========" % (
		...
		for i in range(0, self.numDirectMethods):
			...
			offset += n
			method_idx += method_idx_diff
			appentOutRes( dex_object.getmethodfullname(method_idx, True))
			# print "%s           codeoff=%x"%(dex_object.getmethodname(method_idx),code_off)
			if code_off != 0:
				method_code(dex_object, code_off).printf(dex_object, "\t\t")
		method_idx = 0
...
def get_encoded_value_size(content):
	...
	if value_type in [0x2,3,4,6,0x10,0x11,0x17,0x18,0x19,0x1a,0x1b]:
		...
		appentOutRes( "***************error parse encode_value**************")
	return offset
...
def parse_debug_info(lex_object,offset):
	appentOutRes( "===parse_debug_info====offset = %08x"%offset)
	...
	for i in range(0,parameters_size):
		...
		if string_idx!=-1:
			appentOutRes( lex_object.getstringbyid(string_idx))
		offset+=n
	start = offset
	current_pc = 0
	appentOutRes( "===opcode====offset = %08x  line=%d pc=%d"%(offset,current_line,current_pc))


	totalsize = len(lex_object.m_content)
	while offset < totalsize:
		#bytecode = struct.unpack_from("B",lex_object.m_content,offset)
		bytecode = int(lex_object.m_content[offset])
		offset += 1
		appentOutRes( "opcode[%02x]"%bytecode,False)
		if bytecode == 0:
			appentOutRes( "")
			break
		elif bytecode == 1:
			...
			current_pc += val;
			offset += n
			appentOutRes( "line=%d  pc=%x"%(current_line,current_pc))
		elif bytecode == 2:
			n,val = get_leb128(lex_object.m_content[offset:offset+5])

			current_line += val
			offset += n
			appentOutRes( "line=%d  pc=%x   val=%08x(%d)"%(current_line,current_pc,val,val))
		elif bytecode == 3:
			...
			offset += n
			appentOutRes( "v%d %s %s  START_LOCAL"%(register_num,lex_object.gettypenamebyid(type_idx),lex_object.getstringbyid(name_idx)))
		elif bytecode == 4:
			n,register_num = get_uleb128(lex_object.m_content[offset:offset+5])
			offset += n
			n,name_idx = get_uleb128p1(lex_object.m_content[offset:offset+5])
			...
			offset += n
			appentOutRes( "v%d %s %s   START_LOCAL_EXTENDED"%(register_num,lex_object.gettypenamebyid(type_idx),lex_object.getstringbyid(name_idx)))
		elif bytecode == 5:
			n,register_num = get_uleb128(lex_object.m_content[offset:offset+5])
			offset += n
			appentOutRes( "v%d  END_LOCAL"%register_num)
		elif bytecode == 6:
			n,register_num = get_uleb128(lex_object.m_content[offset:offset+5])
			offset += n
			appentOutRes( "v%d   register to restart"%register_num)
		elif bytecode == 7:
			appentOutRes( "SET_PROLOGUE_END")
			pass
		elif bytecode == 8:
			appentOutRes( "SET_EPILOGUE_BEGIN")
			pass
		elif bytecode == 9:
			n,name_idx = get_uleb128(lex_object.m_content[offset:offset+5])
			appentOutRes( "%s"%lex_object.getstringbyid(name_idx))
			offset += n
		else:
			...
			#offset += 1
			appentOutRes( "line=%d  pc=%x  adjusted_opcode=%d  pc+ %d  line+%d"%(current_line,current_pc,adjusted_opcode,(adjusted_opcode/15),(adjusted_opcode%15)-4))
	appentOutRes( "===parse_debug_info====offset = %08x$"%offset)
def get_encoded_value(content):
	...
	size_type = int(content[0])
	usebyte = 1

	size = size_type >> 5
	type = size_type & 0x1f
	if type not in size_type:
		appentOutRes( "encoded value error!")
	if type == 0 and size == 0:
		...
		if size == 0:
			...
			appentOutRes( "encoded value error! type=short type=%d size=%d"%(type,size))
		usebyte+=size+1
		...
		if size == 0:
			...
			appentOutRes( "encoded value error! type=char type=%d size=%d"%(type,size))
		usebyte+=size+1
	elif type == VALUE_INT:
		if size == 0:
			value,=struct.unpack_from("b",content,1)
			...
			appentOutRes( "encoded value error! type=int type=%d size=%d"%(type,size))
		usebyte+=size+1

	elif type == VALUE_LONG:
		if size > 7:
			appentOutRes( "encoded value error! type=long type=%d size=%d"%(type,size))
		value=content[1:1+size+1]
		usebyte+=size+1
	elif type == VALUE_FLOAT:
		if size > 3:
			appentOutRes( "encoded value error! type=float type=%d size=%d"%(type,size))
		value=content[1:1+size+1]
		usebyte+=size+1
	elif type == VALUE_DOUBLE:
		if size > 7:
			appentOutRes( "encoded value error! type=double type=%d size=%d"%(type,size))
		value = content[1:1+size+1]
		usebyte+=size+1
		...
		if size > 3:
			appentOutRes( "encoded value error! type=type type=%d size=%d"%(type,size))
		value = content[1:1+size+1]
		usebyte+=size+1

	elif type == VALUE_FIELD:
		if size > 3:
			appentOutRes( "encoded value error! type=field type=%d size=%d"%(type,size))
		value = content[1:1+size+1]
		usebyte+=size+1
	elif type == VALUE_METHOD:
		if size > 3:
			appentOutRes( "encoded value error! type=medhod type=%d size=%d"%(type,size))
		value = content[1:1+size+1]
		usebyte+=size+1
	elif type == VALUE_ENUM:
		if size > 3:
			appentOutRes( "encoded value error! type=enum type=%d size=%d"%(type,size))
		value = content[1:1+size+1]
		usebyte+=size+1
	elif type == VALUE_ARRAY:
		if size != 0:
			appentOutRes( "encoded value error! type=encoded_array type=%d size=%d"%(type,size))
		k,value=get_encoded_array(content[1:1+size+1])
		usebyte+=k
	elif type == VALUE_ANNOTATION:
		if size != 0:
			appentOutRes( "encoded value error! type=encoded_annotation type=%d size=%d"%(type,size))
		...
		if size != 0:
			appentOutRes( "encoded value error! type=NULL  type=%d size=%d"%(type,size))
		value="NULL"
...
def parse_encoded_value(lex_object,content,is_root=False):
	...
	if value_type in [0x2,3,4,6,0x10,0x11,0x17,0x18,0x19,0x1a,0x1b]:
		...
		for q in range(0,value_arg+1):
			mm = int(content[offset+q])
			mm <<= 8*q
			sum|=mm
			#sum += ord(content[offset+q])
		if value_type == 0x17:
			appentOutRes( "string@%d"%sum,False)
			appentOutRes( lex_object.getstringbyid(sum),False)
		elif value_type == 0x18:
			appentOutRes( "type@%d"%sum,False)
			appentOutRes( lex_object.gettypename(sum),False)
		elif value_type == 0x19:
			appentOutRes( "field@%d"%sum,False)
			appentOutRes( lex_object.getfieldname(sum),False)
		elif value_type == 0x1a:
			appentOutRes( "method@%d"%sum,False)
			appentOutRes( lex_object.getmethodname(sum),False)
		else:
			str = ""
			for q in range(0,value_arg+1):
				str += "%02x "%(int(content[offset+q]))
		...
		appentOutRes( "NULL",False)
	elif value_type == 0x1f:
		if value_arg == 0:
			appentOutRes( "False",False)
		else:
			appentOutRes( "True",False)
		offset += 0
	elif value_type == 0x1d:
...
def parse_encoded_value1(lex_object,content,is_root=False):
	...
	if value_type in [0x2,3,4,6,0x10,0x11,0x17,0x18,0x19,0x1a,0x1b]:
		...
		if value_type == 0x17:
			...
			str2 = ""
			for q in range(0,value_arg+1):
				str2 += "%02x "%(int(content[offset+q]))
			str1+= str2
		offset += (value_arg+1)
	elif value_type == 0:
		str1 += "%02x"%int(content[offset])
		offset += 1
...
def parse_encoded_annotation(lex_object,content,is_root=False):
	...
	offset += n
	if is_root:
		appentOutRes( lex_object.gettypenamebyid(type_idx),False)
	for i in range(0,size):
		n ,name_idx = get_uleb128(content[offset:5+offset])
		if i == 0 and is_root:
			appentOutRes( lex_object.getstringbyid(name_idx),False)
		offset += n
...
def parse_annotation_set_item(lex_object,offset,is_root=False):
	try:
		...
		for i in range(0,size):
			...
			if visibility == 0:
				appentOutRes( "VISIBILITY_BUILD",False)
			elif visibility == 1:
				appentOutRes( "VISIBILITY_RUNTIME",False)
			elif visibility == 2:
				appentOutRes( "VISIBILITY_SYSTEM",False)
			else:
				appentOutRes( "visibility is unknow %02x"%visibility)
...
class dex_parser:
	def __init__(self,filename):
		...
		if self.m_content[0:4].decode(encoding="utf-8") == DEX_OPT_MAGIC:
			self.init_optheader(self.m_content)
			self.init_header(self.m_content,0x40)
		elif self.m_content[0:4].decode(encoding="utf-8") == DEX_MAGIC:
			self.init_header(self.m_content,0)
		else:
			appentOutRes("dex格式不正确,请检查dex文件")
	...
	def getstringbyid(self,stridx):
		if stridx >= self.m_stringIdsSize:
			return ""
		try:
			res=self.string_table[stridx].decode(encoding="utf-8")
		except:
			res =b2hexSpace(self.string_table[stridx])
		return res
	...
	def getmethodname(self,methodid):
		...
		return self.string_table[name_idx].decode(encoding="utf-8")
	...
	def gettypename(self,typeid):
		...
		return self.string_table[descriptor_idx].decode(encoding="utf-8")
	...
	def getprotoname(self,protoid):
		...
		return self.string_table[shorty_idx].decode(encoding="utf-8")
...
# def fartOpStart(dexfilename,insfilename):
# 	methodTable.clear()
# 	parseinsfile(insfilename)
# 	appentOutRes("methodTable length:" + str(len(methodTable)))
# 	# dex = dex_parser(dexfilename)

class FartThread(QThread):
	loggerSignel = pyqtSignal(str)
	def __init__(self, dexfilename,insfilename):
		super(FartThread, self).__init__()
	...
	def run(self):
		global LOG_SIGNEL
		LOG_SIGNEL = self.loggerSignel
		methodTable.clear()
		parseinsfile(self.insfilename)
		appentOutRes("methodTable length:" + str(len(methodTable)))
		dex = dex_parser(self.dexfilename)